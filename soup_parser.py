...
class SoupContentParser(object):
    ...
    def get_phone(self, soup_content):
        try:
            ...
            for data in soup_content.find_all("div", {"class": "card-phones-view__number"}):
                phone = data.getText().replace("'", "`")
                phones.append(phone)
            return phones
        except Exception:
            return []

    def amount_reviws(self, soup_content):
        try:
            amount_of_reviews = 0
            link = None
            for data in soup_content.find_all("div", {"class": "tabs-select-view__title"}):
                if "отзыв" in data.text.lower():
                    link = data.find("a", {"class": "tabs-select-view__label"}).get("href")
                    amount_of_reviews = int(data.find("div", {"class": "tabs-select-view__counter"}).text)
            return amount_of_reviews, link
    ...
    def get_rating(self, soup_content):
        ...
        try:
            ...
            try:
                rating = float(soup_content.find("span", {"class": "business-rating-badge-view__rating-text"}).text.replace(',','.'))
                return rating
            except:
                return range(random.uniform(2,4), 1)

    def get_logo(self, soup_content):
        try:
            logo = soup_content.find("img", {"class": "img-with-alt"}).get("src")
            return logo
        except Exception:
            return None
